...
function main() {
  ...
  for (n = mineSaker.length - 1; n >= 0; n -= 1) {
    // sorter ut hvilke saker som er aktuelle å koble bilder til
  ...
  mineSaker = [
    {
      value: true,
      arbeidstittel: 'IKKE KOBLE TIL NOEN SAK',
      tekst:
        'hvis du velger dette blir ikke bildet koblet til produksjonssystemet',
      mappe: ''
    }
  ].concat(mineSaker.reverse()) // legger til et valg i starten av lista
  ...
  sakstittel.onChange = function() {
    ...
    sakstittel.text = sakstittel.text
      .replace(/æ/g, 'a')
      .replace(/ø/g, 'o')
      .replace(/å/g, 'a')
      .replace(/[^a-z0-9]/g, '') // kutter ut ikke-norske bokstaver og ikke-alfanumeriske tegn
    filnavn(1)
  }
  mappenavn.onChange = function() {
    var myRegExp = new RegExp('^' + mappenavn.text, 'i')
    folderexists = false
    for (n = 0; n < seksjoner.length; n += 1) {
      // går gjennom seksjonene for å se om de matcher med det som er skrevet i feltet
  ...
  for (var n = 0; n < panelSize; n += 1) {
    ...
    myrow.text2 = myrow.add(
      'statictext',
      undefined,
      mineSaker[n].arbeidstittel.substr(0, 38)
    )
      ...
      myrow.button.onClick = (function(sak, button) {
        return function() {
          ...
          if (valgtsak) {
            ...
            sakstittel.text = valgtsak.arbeidstittel
              .toLowerCase()
              .replace(/^[\d\s]+/, '')
              .replace(/æ/g, 'a')
              .replace(/ø/g, 'o')
  ...
  if (mineSaker.length > panelSize) {
    panel1.scrollbar = panel1.add(
      'scrollbar',
      undefined,
      0,
      0,
      mineSaker.length - panelSize
    )
  ...
  for (n = 0; n < mydocuments.length; n += 1) {
    ...
    label = myrow.add(
      'statictext',
      undefined,
      mydok.name.substr(0, 25),
      undefined
    )
    label.graphics.font = smallfont
    filer[n].filnavn = myrow.add(
      'edittext',
      undefined,
      (mydok.name + '.').replace(/\..*/, '.jpg').replace(/\s+/g, dash)
    )
    ...
    filer[n].filnavn.onChange = (function(myedittext) {
      return function() {
        myedittext.text = (myedittext.text + '.')
          .replace(/\..*/, '.jpg')
          .replace(/\s+/g, dash)
      }
    })(filer[n].filnavn)
    ...
    label.helpTip =
      'skriv inn navn på personer, steder og så videre for å gjøre bildet søkbart'
    label.graphics.font = smallfont
  ...
  if (svar == 1) {
    ...
    for (n = 0; n < filer.length; n += 1) {
      ...
      if (minfil.checkbox.value === true && minfil.fil) {
        ...
        if (mydok.layers.length == 1 && mydok.layers[0].isBackgroundLayer) {
          // er dokumentet flattened?
          mydok.saveAs(fila, jpgsettings) // save as
        } else {
          mydok.saveAs(fila, jpgsettings, true) // save as copy
        }
        json +=
          '{"prodbilde_id":"0","bildefil":"' +
          fila.name +
          '","bildetekst":"' +
          bildetekst +
          '","prioritet":"' +
...
function errorAlert(message, title) {
  ...
  d.bold.graphics.font = ScriptUI.newFont(
    f.name,
...
function utgave() {
  // finner ut hvilket nummer som er det neste ved å lete etter mappa med høyest nummer
  var path = rotmappe
  var nummer
  var myFile
  for (i = 50; i > 0; i--) {
