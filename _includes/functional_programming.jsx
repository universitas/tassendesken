...
function pipe() {
  var fnx = arguments
  return function(value) {
    for (var i = 0; i < fnx.length; i++) value = fnx[i](value)
    return value
  }
}
...
function keys(obj) {
  var retval = []
  for (var key in obj) if (obj.hasOwnProperty(key)) retval.push(key)
  return retval
}

function values(obj) {
  var retval = []
  for (var key in obj) if (obj.hasOwnProperty(key)) retval.push(obj[key])
  return retval
}
...
function zip(collections) {
  ...
  retval = []
  for (i = 0; i < length; i++) retval.push(pluck(i)(arrs))
  return retval
}
