...
var testing = false; // hvis true flyttes ikke saker til neste nivå i prodsys. Testing kan også være et prodsak_id - i såfall brukes ikke dialogboksen
var finnbilder = true; // siden det tar så lang tid å finne bilder, kan det skrus av når man driver med bugtesting av skriptet
...
function importerSak(JSONsak, somArtikkelType, importerbilder){ // JSONsak er et objekt basert på prodsys-json. somArtikkelType er et artikkeltypeobjekt
	finnbilder = importerbilder; // Skal bilder importeres. Kan skrus av for å teste eller spare litt tid
	...
	if (testing){ // brukes bare ved uttesting av skriptet, ikke i vanlig produksjon
...
function artikkel(JSONsak, somArtikkelType, mySpread, mySelection) { // det viktigste objektet i dette skriptet. Inneholder masse funksjoner og verdier - opprettes på grunnlag av en string fra prod.sys
	this.main = function(){ // hovedfunksjon - kaller en drøss subrutiner i rekkefølge
		if(JSONsak==null){exit();} // avslutter hele skriptet hvis den opprinnelige JSON-importen fra prodsys ikke resulterer i en gyldig string
		...
		this.pageItems = {textFrames:[], rectangles:[], cells:[]}; // en objekt som innehodler arrays av tekstrammer, bilderammer og tabellceller som saken blir limt inn i
		...
		// TODO: gjør til subfunksjon
			...
			for (var n=0; n<this.bildetekster.length; n++){ // hvis det er flere bildetekster enn bilder
				this.bilder.push({"prodbilde_id":null,"bildefil":null,"bildetekst":this.bildetekster[n],"prioritet":"5","prodsak_id":this.id}); // lager bilde som har tekst men ikke noen bildefil tilkobla
			}
		// TODO 
		...
		if (this.artikkeltype.bylineboks){ // skal denne artikkeltypen ha bylineboks?
			...
			if (trynebilde){
				this.skjema.pictures.push({rectangle:trynebilde}); // legger trynebildet i bilde-skjemat
			}
		}
		...
		if (JSONsak.tekst.match(/@tit:/)&&JSONsak.tekst.match(/@tit:/g).length>1){ // er det mer enn én @tit - tag ?
			...
			if (sakstype && undersaker.length>1){
				for (var n = 2; n < undersaker.length; n++){
					...
					for (i = 1; i < myTags.length; i++){
						myTag = myTags[i].match(/[^:]+/);
						if (myTag){
							if (null==myTag[0].match(undersakstags)){ // avsnittet skal ikke være i undersaken, selv om det kommer etter undersakstittelen i råteksten (gjelder for eksempel uthevet sitat, faktaramme eller byline)
								underJSON.tekst = underJSON.tekst.replace("@"+myTags[i], "");						
							}else{
								JSONsak.tekst = JSONsak.tekst.replace("@"+myTags[i],"");
							}
						}
					}
	...
	this.parseParagraphs = function (pasteString){ // deler artikkelen opp i en array med objekter som inneholder tekststring og paragraphstyle
		...
		for (i=0;i<avsnittsamling.length;i++){
			...
			if (stil.match(/bl/)&&this.bylineboksInnhold){ // byline
				...
				paragraphs.push({contents: mittAvsnitt, style: stil});
			}
		}
		return paragraphs;
	}			
	...
	this.plasserbilder = function(mineBilder){ // limer bilder inn i rectangles
		...
		while (this.bilder.length>mineBilder.length){
			...
			mineBilder.push({"rectangle":bildeRamme, "bt":btRamme});
		...
		for (var q = 0; q < this.bilder.length; q+=1){
			...
			if (finnbilder&&this.bilder[q].bildefil){ // finnbilder kan skrus av for testing
	...
	this.opprydding = function(){
		...
		for (var n=0;n<this.pageItems.cells.length;n++){
			minCelle = this.pageItems.cells[n];
		...
		for (var n=0;n<this.pageItems.rectangles.length;n++){
			mittBilde = this.pageItems.rectangles[n];
...
function applyItalic(myStory){ // applies Italics may take a story, else does it for the entire document (slower);
	...
	if (myStory) {
		...
		var myRanges = myDocument.stories.everyItem().textStyleRanges.everyItem().getElements(); // gets every textStyleRange in the document;
	}
	...
	for (n=0; n<myRanges.length; n++){
		...
		if (myRange.appliedCharacterStyle==italicStyle){
			...
			if (newStyle==null){
				newStyle=myDocument.characterStyles.add({name: newStyleName, fontStyle: myFont.fontStyleName}); // if the style does not exist yet;
			}
...
function lagSkjema(myLabel, mySelection, mySpread){ // finner ut hvilke stiler og bilder som skal hvor ut i fra en selection som består av en gruppe eller tekstramme
	var mittSkjema={pictures:[], stories:[], styles:{}}; // alle bilde
	...
	var findAllElements = function(myGroup,mittSkjema){
		var findElements = function(myStory){
			...
			for (var m=0;m<myStory.pageItems.length;m++){
		...
		if (myGroup instanceof Group){
			loop:
			for (var n=0 ; n<myGroup.textFrames.length ; n++){ // går gjennom alle textFrames i gruppa
				for (var m=0 ; m<n ; m++){
			...
			for (n=0;n<myGroup.rectangles.length;n++){ // går gjennom alle rectangles (bilderammer) i gruppa
				if (myGroup.rectangles[n].contentType == ContentType.GRAPHIC_TYPE){
					mittSkjema.pictures.push(myGroup.rectangles[n].id);
				}
			}
			myGroup.ungroup();
	...
	var mittSkjema = findAllElements(myGroup, mittSkjema);
...
if (testing.constructor.name=="Number"){ // testing kan være en prodsak_id
	...
	var testSakstype;
	var erTtype;
	var minSak = prodsys.get(testing);
	...
	importerSak(minSak,minSakstype,finnbilder); // importerer saken og plasserer den på siden.
}else{	
	importPanel.visDialog(testing);
}
