...
if (!alleGrupper){
	...
	for (i=myPageItems.length-1; i>-1; i--){
		...
		if (myItem instanceof Group || myItem instanceof TextFrame){
			while (false == (mySpread instanceof Spread || mySpread instanceof MasterSpread || mySpread instanceof Application)){
...
function lagNyAvisMal(){
	...
	if (!myDok.name.match(/saksmaler/i)){
		...
		var myProgressBar = 	dokTools.progressBar("Legger maler til " + lagreSom.name, "Lagrer Saksmaler.indd", 2+avismal.length, false);
