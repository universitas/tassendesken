...
if (!alleGrupper){
	...
	for (var i = 0; i<myItems.length-1; i++){		
		...
		if (myGroup.label!=""){
			...
			if (myGroup instanceof TextFrame){ itemName += myGroup.contents.slice (0, 10)};
		}
...
function mekkMalGeo(mySelection, promptMe){
	...
	for (var n = mySelection.length-1; n>=0 ; n--){
		if (false == (mySelection[n].parent instanceof Page || mySelection[n].parent instanceof Spread)){
			mySelection.splice (n, 1);
		}
	}
	if (mySelection.length == 0){
		return;
...
function tagReport(myTextFrames, sakstype){
	...
	var makeReport = function(){
		...
		for (var n=0; n<paragraphStyles.length; n++){
			myReport = myReport+paragraphStyles[n]+"\r"
		}
		return myReport;
	}
	...
	var getXtags = function(eksportStories){
		...
		for (var n=0; n<myStories.length;n++){
			myXtagsString=myXtagsString+getXtagStory(myStories[n].story)
		}
...
function lagNyAvisMal(){
	var avismal = [ // sider som skal være i MAL_AVIS.indt
		{master: "F-Front", tomside: false},
		{master: "2-Side 2-3", tomside: false},
		{master: "N-Nyhet Intro", tomside: false},
		{master: "N-Nyhet", tomside: true},
		{master: "N-Nyhet", tomside: true},
		{master: "N-Nyhet", tomside: true},
		{master: "D-Debatt Intro", tomside: false},
		{master: "K-Kultur Intro", tomside: true},
		{master: "K-Kultur", tomside: true},
		{master: "K-Kultur", tomside: true},
		{master: "K-Reportasje", tomside: true},
		{master: "P-Plakat", tomside: false},
		{master: "B-Baksida", tomside: false}
	]
